...
class Application:
    ...
    def handle_gesture(self, char):
        ...
        # send sentence to Unity
        if self.sentence != self.last_sent_sentence:
            try:
                sock.sendto(self.sentence.encode(), server_address)
                self.last_sent_sentence = self.sentence
            except:
                pass
